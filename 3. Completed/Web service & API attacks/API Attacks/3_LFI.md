...
* We don't see anything helpful except the indication that the API is up and running
* Let us perform API endpoint fuzzing using _ffuf_ and the [common-api-endpoints-mazen160.txt](https://github.com/danielmiessler/SecLists/blob/master/Discovery/Web-Content/common-api-endpoints-mazen160.txt) list
...
* It looks like `/api/download` is a valid API endpoint
...
* We need to specify a file, but we do not have any knowledge of stored files or their naming scheme
* We can try mounting a Local File Inclusion (LFI) attack
