...
* Remote Desktop Protocol (`RDP`)
	* Protocol to remotely access a computer running Windows
	* Allows display and control commands to be transmitted via GUI encrypted over IP networks
	* Typically on TCP port `3389` for transport
	* UDP can use port `3389` as well for remote administration
	*  If [Network Address Translation](https://en.wikipedia.org/wiki/Network_address_translation) (`NAT`) is used on the route between client and server, as is often the case with Internet connections, the remote computer needs the public IP address to reach the server
...
* We can use [rdp-sec-check.pl](https://github.com/CiscoCXSecurity/rdp-sec-check) tool to enumerate RDP even further
...
- Windows Remote Management (`WinRM`)
	...
	- WinRM relies on `TCP` ports `5985 (HTTPS)`  and `5986 (HTTPS)` for communication
	- Windows Remote Shell (`WinRS`)
		- Lets us execute arbitrary commands on the remote system 

### Footprinting the Service
* We will see that only HTTP (`TCP 5985`) is used instead of HTTPS (`TCP 5986`)
...
* Extension of the Common Information Model (`CIM`), core functionality of the standardized Web-Based Enterprise Management (`WBEM`) for the Windows platform
...
* Consists of several programs and various databases, also known as repositories

### Footprinting the Service
*  TCP port `135`
* The program [wmiexec.py](https://github.com/SecureAuthCorp/impacket/blob/master/examples/wmiexec.py) from the Impacket toolkit can be used
