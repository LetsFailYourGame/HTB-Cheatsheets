...
* `Internet Message Access Protocol` (`IMAP`)
	...
	* Supports folder structures and synchronization of a local email client with the mailbox on the server
	* Client-server based
	* Text-based commands in ASCII
	* Multiple users can access the email server simultaneously
	* Client connects via port `143`
	* Unencrypted but SSL/TLS can be forced by servers
		* When encrypted either port `143` or `993` is used
* `Post Office Protocol` (`POP3`)
	* Does not have same functions as IMAP
	* Only provides listing, retrieving, and deleting emails

### IMAP Commands
![](./Screenshots/Screenshot_2022-10-04_183133.png)

### POP3 Commands
![](./Screenshots/Screenshot_2022-10-04_183247.png)

* Use `FETCH BODY[]` to see the body of a message

### Dangerous Settings
* Most companies use third-party email providers such as Google, Microsoft, and many others
	* Some still use their own mail server for many reasons
	* Many configuration mistakes can happen

	![](./Screenshots/Screenshot_2022-10-04_183449.png)

### Footprinting the Service
* By default, ports `110`, `143`, `993`, and `995` are used for IMAP and POP3
	* `993` and `995` use `TLS/SSL` to encrypt communication
...
* We can see that the common name is `mail1.inlanefreight.htb` 
* Also the email server belongs to the organization `Inlanefreight` located in California
* The capabilities show us the commands available on the server and for the service on the corresponding port
* If we would have credentials from an employee we could log in into the mail server
...
* If we use the `verbose` (`-v`) option, we can further enumerate the `TLS version`, `SSL certificate`, and even the banner which contains the `version of the mail server`
...
* To interact with IMAP or POP3 we can use `openssl` or `netcat`
